...
export const ALL_MENU_OPTIONS: MenuOption[] = [
  ...
  { id: "/attachments", label: "Attachments", meta: "Attachments queue.", kind: "screen", path: "/attachments" },
...
function getLabelOverrides(config?: AppConfig) {
  return {
    ...
    attachmentsMeta: config?.ui?.navigation?.attachments?.meta ?? "Attachments queue",
    itemstoreviewMeta: config?.ui?.navigation?.itemstoreview?.meta ?? "Items to Review workspace",
...
export function getMenuOptions(config?: AppConfig) {
  ...
  return ALL_MENU_OPTIONS.map((option) => {
    ...
    if (option.id === "/itemstoreview") {
      return { ...option, label: labels.itemstoreview, meta: labels.itemstoreviewMeta };
    }
